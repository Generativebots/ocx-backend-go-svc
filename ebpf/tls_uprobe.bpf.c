// eBPF uProbes for TLS Interception
// Hooks OpenSSL, BoringSSL, and Go crypto/tls to capture plaintext
...
int BPF_KRETPROBE(uretprobe_ssl_read, int ret) {
    ...
    // Read plaintext data
    bpf_probe_read_user(event.data, len, (void *)*buf_ptr);
...
// Note: Go uses different calling conventions, requires offset analysis
...
int BPF_KPROBE(uprobe_go_tls_write) {
    ...
    // Go-specific: extract data from stack/registers
    // This is simplified; real implementation needs Go struct analysis
