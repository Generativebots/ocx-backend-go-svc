// eBPF PID-to-Identity Mapper
// Provides kernel-level identity verification for agent processes
...
struct identity_t {
    ...
    __u64 spiffe_svid_hash;      // Hash of SPIFFE SVID for verification
    __u64 registered_at;         // Timestamp when registered
    __u32 parent_pid;            // Parent PID for tracking
};
...
// Perf event array for notifications
struct {
...
// Hook: Process fork - propagate identity to child
...
int trace_fork(struct trace_event_raw_sched_process_fork *ctx) {
    ...
    if (!parent_id) {
        return 0;  // Parent has no identity, skip
    }
...
// Hook: Process exec - maintain identity across exec
...
int trace_exec(struct trace_event_raw_sched_process_exec *ctx) {
    ...
    if (!id) {
        return 0;  // No identity, skip
    }
    
    // Identity persists across exec (the "taint" follows)
    // Send event to userspace
    struct identity_event event = {};
    event.pid = pid;
...
// Hook: Process exit - cleanup identity
...
// Kprobe: Capture socket connect for identity attribution
...
int BPF_KPROBE(kprobe_tcp_connect, struct sock *sk) {
    ...
    // Send lookup event (for SOP to verify identity)
    struct identity_event event = {};
    event.pid = pid;
    event.parent_pid = id->parent_pid;
